...
if (file_put_contents($petitionsFile, json_encode($petitions, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
    ...
    $emailSent = EmailManager::sendPetitionFormNotification($petitionData);
    
    if ($emailSent) {
        echo json_encode([
            'success' => true,
            'message' => 'Petiția dumneavoastră a fost trimisă cu succes! Veți primi o confirmare pe email cu toate detaliile și termenii de răspuns.'
        ]);
    } else {
        echo json_encode([
            'success' => true,
            'message' => 'Petiția dumneavoastră a fost trimisă cu succes! Vă vom contacta conform termenilor legali. (Notă: Confirmarea pe email poate întârzia din motive tehnice.)'
        ]);
    }
} else {
    http_response_code(500);
